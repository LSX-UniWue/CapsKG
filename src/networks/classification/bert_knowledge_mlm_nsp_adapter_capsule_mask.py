...
from transformers.models.bert.modeling_bert import BertOnlyMLMHead, BertOnlyNSPHead, BertForMaskedLM, BertForNextSentencePrediction
import utils
...
class Net(torch.nn.Module):
    ...
    def __init__(self,taskcla,args):
        ...
        for t,n in self.taskcla: # TODO my code add mask prediction head for each KG
            ...
            kg_next_sentence_head_layer.load_state_dict(original_model_for_nsp_pred.cls.state_dict())

            self.last_mlm.append(kg_maskhead_layer)
            self.last_nsp.append(kg_next_sentence_head_layer)
    ...
    def forward(self,t,input_ids, segment_ids, input_mask, targets, s=1):
        ...
        for t,i in self.taskcla:
            y.append(self.last_mlm[t](sequence_output)) # TODO my code masking task, input is sequence

        nsp = []
        for t,i in self.taskcla:
            nsp.append(self.last_nsp[t](pooled_output)) # TODO my code nsp task, input is pooled outpus. see HF BERT
