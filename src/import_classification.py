...
language_dataset = ['asc','dsc','ssc','nli','newsgroup', 'wn18', 'yago', 'fbl']
...
elif args.task == 'wn18': #document sentiment classication # TODO my code
    if args.backbone=='w2v':
        from dataloaders.wn18 import w2v as dataloader
    elif args.backbone in bert_backbone:  # all others
        from dataloaders.wn18 import bert as dataloader

elif args.task == 'yago': #document sentiment classication # TODO my code
    if args.backbone=='w2v':
        from dataloaders.yago import w2v as dataloader
    elif args.backbone in bert_backbone:  # all others
        from dataloaders.yago import bert as dataloader


elif args.task == 'fbl': #document sentiment classication # TODO my code
    if args.backbone=='w2v':
        from dataloaders.fbl import w2v as dataloader
    elif args.backbone in bert_backbone:  # all others
        from dataloaders.fbl import bert as dataloader
...
if args.task in language_dataset:
    ...
    if args.backbone == 'bert':
        ...
        if args.baseline=='one_mlm':
            from approaches.classification import bert_one_mlm as approach
            from networks.classification import bert_mlm as network
        if args.baseline=='one_bin':
            from approaches.classification import bert_one_bin as approach
            from networks.classification import bert as network
        if args.baseline=='one_mlm':
            from approaches.classification import bert_one_mlm as approach
            from networks.classification import bert_mlm as network
        if args.baseline=='ncl':
            ...
            from networks.classification import bert as network
        if args.baseline=='ncl_mlm':
            from approaches.classification import bert_ncl_mlm as approach
            from networks.classification import bert_mlm as network
        if args.baseline == 'mtl':
            from approaches.classification import bert_mtl as approach
            from networks.classification import bert as network
        if args.baseline == 'mtl_mlm':
            from approaches.classification import bert_mtl_mlm as approach
            from networks.classification import bert_mlm as network
    ...
    if args.backbone == 'bert_adapter':
        if args.baseline == 'derpp':
            ...
            from networks.classification import bert_adapter as network
        elif args.baseline=='one_mlm':
            from approaches.classification import bert_adapter_one_mlm as approach
            from networks.classification import bert_adapter_mlm as network
        elif args.baseline=='ncl':
            from approaches.classification import bert_adapter_ncl as approach
            from networks.classification import bert_adapter as network
        elif args.baseline=='ncl_mlm':
            from approaches.classification import bert_adapter_ncl_mlm as approach
            from networks.classification import bert_adapter_mlm as network
        elif args.baseline == 'ucl':
        ...
        elif args.baseline=='ctr_kg': # TODO my knowledge capsule mlm head
            from approaches.classification import bert_knowledge_adapter_capsule_mask as approach
            from networks.classification import bert_knowledge_adapter_capsule_mask as network
