...
class Appr(object):
    ...
    def eval(self,t,data,test=None,trained_task=None, filter_eval=False):
        ...
        with torch.no_grad():
            for step, batch in enumerate(data):
                ...
                loss=self.criterion(output.view(-1, self.model.tokenizer.vocab_size),targets.view(-1))
                if filter_eval:
    ...
    def criterion_train(self,tasks,outputs,targets):
        ...
        for t in np.unique(tasks.data.cpu().numpy()):
            ...
            loss+=self.criterion(output[idx,:].view(-1, self.model.tokenizer.vocab_size),targets[idx].view(-1))*len(idx)
