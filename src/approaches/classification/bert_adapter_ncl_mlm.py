    ...
    def train(self,t,train,valid, test, num_train_steps,train_data,valid_data):
        ...
        for e in range(int(self.args.num_train_epochs)):
            ...
            if self.args.save_model:
                path = self.args.model_save_path + 'model_after_epoch_' + str(e) + '_for_task_'+str(t)+'_' + datetime.now().strftime(
                    "%Y-%m-%d_%H-%M-%S")
                torch.save(self.model, path)
                torch.save(self.model.state_dict(), path + '_state_dict')
        ...
        with torch.no_grad():
            for step, batch in enumerate(data):
                ...
                if filter_eval:
                    ...
                    # non_filtered_pred =  output[(targets != -100).nonzero(as_tuple=True)].argmax(1)
                    pred = filtered_output.argmax(1)
                    # topk_20_probs, topk_20_ids = filtered_output.softmax(1).topk(20)
                else:
                    pred = output.argmax(2)[(targets != -100).nonzero(as_tuple=True)]
                    # topk_20_probs, topk_20_ids = output[(targets != -100).nonzero(as_tuple=True)].softmax(1).topk(20)


                _,pred_old = output.max(1)
