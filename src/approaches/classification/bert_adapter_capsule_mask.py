...
class Appr(ApprBase):
    ...
    def train_epoch(self, t, data, iter_bar, optimizer, t_total, global_step):
        ...
        for step, batch in enumerate(iter_bar):
            ...
            if t > 0:
                for n, p in self.model.named_parameters():
                    # TODO my: All TSM layers get gradients set to zero on neurons that are used by previous tasks
                    if n in self.mask_back and p.grad is not None:
                        p.grad.data *= self.mask_back[n]
                    # TODO my: All tsv routing layers get gradients set to zero on tasks that have not been trained yet (through triangular matrix)
